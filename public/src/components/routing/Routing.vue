                    ...
                    <el-col :span="8">
                        <el-form-item label="tag">
                            <el-input v-model="b.tag" v-setting >
                                <el-button slot="append" icon="el-icon-delete" @click="sForm.balancers.splice(idx,1)" />
                            </el-input>
                        </el-form-item>
                    </el-col>
                    <el-col :span="16">
        ...
        <setting-card title="规则列表" :show-enable="false">
            ...
            <transition-group name="rule-list" tag="div">
                <Rule v-for="(rule,idx) in sForm.rules" :idx="idx" :rule="rule"
                      @new-rule="newRule" @del-rule="delRule" v-setting
        ...
        methods: {
            fillDefaultValue(setting) {
                ...
                balancers.forEach(balancer=>{
                    if(typeof(balancer.id)==="undefined"){
                        balancer.id = ruleIdx++;
                    }
                });
            },
